...
pub trait VerifiableEncryption: BulletproofCurveArithmetic {
    ...
    fn blind_encrypt_and_prove(
        ...
        generator: Option<Self::Point>,
        rng: impl RngCore + CryptoRng,
    ) -> (Ciphertext<Self>, Proof<Self>) {
        let blinded_key_share = *key_share + blinder;
        let (ciphertext, proof) = Self::encrypt_and_prove(
            encryption_key,
            &blinded_key_share,
            authenticated_data,
            generator,
    ...
    fn encrypt_and_prove(
        ...
        let generator = generator.unwrap_or_else(|| Self::Point::generator());
        ...
        for i in 0..key_bytes.len() {
            ...
            let r1 = generator * blinder_blinders[i];
        ...
        let dlog_committing = DlogProof::<Self>::create(
            encryption_key,
            key_share,
            &r,
            generator,
    ...
    fn verify(
        ...
        for i in 0..key_bytes {
            ...
            let r1 = ciphertext.c1[i] * proof.challenge + generator * proof.byte_proofs[i].blinder;
            let r2 = ciphertext.c2[i] * proof.challenge
                + encryption_key * proof.byte_proofs[i].blinder
                + generator * proof.byte_proofs[i].message;
            transcript.append_point::<Self>(b"elgamal_segment_proofs_r1", &r1);
            transcript.append_point::<Self>(b"elgamal_segment_proofs_r2", &r2);
        }

        transcript.append_point::<Self>(b"G", &generator);
        ...
        // (r1 + c.x).G
        let lhs1 = generator * proof.dlog_proof.message;
        // r1.G + c.x.G
        ...
        // (r2 + c.r).G
        let lhs3 = generator * proof.dlog_proof.blinding;
        // r2.G + c.r.G
...
pub trait VerifiableEncryptionDecryptor: BulletproofCurveArithmetic {
    ...
    fn decrypt_and_verify(
        ...
        let plaintext = Self::decrypt(decryption_key, ciphertext, generator)?;
        let generator = generator.unwrap_or_else(|| Self::Point::generator());
        if proof.dlog_proof.c2 - proof.dlog_proof.c1 * decryption_key == generator * plaintext {
            Ok(plaintext)
        } else {
    ...
    fn decrypt_and_unblind(
        ...
        let blind_plaintext = Self::decrypt(decryption_key, ciphertext, generator)?;
    ...
    fn decrypt_with_shares_and_unblind(
        ...
        let blind_plaintext = Self::decrypt_with_shares(decryption_shares, ciphertext, generator)?;
        Ok(blind_plaintext - blinder)
    }
...
fn blind_encrypt_and_prove_ristretto25519_works() {
    ...
    blind_encrypt_and_prove_works::<Ristretto25519>(signing_key, WrappedRistretto::generator());
}
...
fn blind_encrypt_and_prove_jubjub_works() {
    ...
    blind_encrypt_and_prove_works::<JubJub>(signing_key, SubgroupPoint::generator());
}
...
fn blind_encrypt_and_prove_jubjub_alt_works() {
    ...
    let pt: ExtendedPoint = AffinePoint::from_bytes(&SPENDAUTHSIG_BASEPOINT_BYTES)
        .unwrap()
        .into();
...
fn blind_encrypt_and_prove_ed25519_works() {
    ...
    blind_encrypt_and_prove_works::<Ed25519>(signing_key, WrappedEdwards::generator());
}
...
fn blind_encrypt_and_prove_bls12381_works() {
    ...
    blind_encrypt_and_prove_works::<Bls12381G1>(signing_key, G1Projective::GENERATOR);
}
...
fn blind_encrypt_and_prove_works<C: VerifiableEncryption + VerifiableEncryptionDecryptor>(
    ...
    let encryption_key = generator * decryption_key;
    ...
    let share_verification_key = generator * share1;
    ...
    assert!(res.is_err());
    let share_verification_key = generator * (share1 + blinder);
    let res = C::verify(
        encryption_key,
        share_verification_key,
        &ciphertext,
    ...
    let res = C::decrypt(&decryption_key, &ciphertext, Some(generator));
    assert!(res.is_ok());
    assert_ne!(res.unwrap(), share1);

    let res = C::decrypt_and_verify(&decryption_key, &ciphertext, &proof, Some(generator));
    assert!(res.is_ok());
    assert_ne!(res.unwrap(), share1);

    let res = C::decrypt_and_unblind(&blinder, &decryption_key, &ciphertext, Some(generator));
    assert!(res.is_ok());
...
fn blind_encrypt_and_proof_random_ids_bls12381_works() {
    ...
    let (ciphertext, proof) =
        Bls12381G1::blind_encrypt_and_prove(encryption_key, &share1, &blinder, &[], None, &mut rng);
    ...
    let res = Bls12381G1::decrypt(&decryption_key, &ciphertext, None);
    assert!(res.is_ok());
    assert_ne!(res.unwrap(), share1);

    let res = Bls12381G1::decrypt_and_verify(&decryption_key, &ciphertext, &proof, None);
    assert!(res.is_ok());
    assert_ne!(res.unwrap(), share1);

    let res = Bls12381G1::decrypt_and_unblind(&blinder, &decryption_key, &ciphertext, None);
    assert!(res.is_ok());
    assert_eq!(res.unwrap(), share1);
...
fn encrypt_and_prove_ristretto25519_works() {
    ...
    encrypt_and_prove_works::<Ristretto25519>(signing_key, WrappedRistretto::generator());
}
...
fn encrypt_and_prove_ed25519_works() {
    ...
    encrypt_and_prove_works::<Ed25519>(signing_key, WrappedEdwards::generator());
}
...
fn encrypt_and_prove_bls12381_works() {
    ...
    encrypt_and_prove_works::<Bls12381G1>(signing_key, G1Projective::GENERATOR);
}
...
fn encrypt_and_prove_p384_works() {
    ...
    encrypt_and_prove_works::<NistP384>(signing_key, ProjectivePoint::GENERATOR);
}
...
fn encrypt_and_prove_ed448_works() {
    ...
    encrypt_and_prove_works::<Ed448>(signing_key, EdwardsPoint::GENERATOR);
}
...
fn encrypt_and_prove_works<C: VerifiableEncryption + VerifiableEncryptionDecryptor>(
    ...
    let share1 = shares[0].value.0;
    let (ciphertext, proof) =
        C::encrypt_and_prove(encryption_key, &share1, &[], Some(generator), &mut rng);
    let share_verification_key = generator * share1;
